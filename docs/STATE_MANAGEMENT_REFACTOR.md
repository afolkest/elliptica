...
- [x] Delete `expr_pending_update`, `expr_last_update_time` (kept `expr_debounce_delay` — shared with 4d)
...
- [x] Kept `_cached_global_lightness_expr`, `_cached_custom_lightness_exprs` as UI draft memory (not state caches — store values removed from AppState for toggle UX)
